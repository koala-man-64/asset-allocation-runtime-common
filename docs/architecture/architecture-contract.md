...
| Canonical Baseline | `main` branch, package version `3.4.1` in `python/pyproject.toml` |
...
Consumer repos must consume this package as a versioned dependency rather than through sibling source checkout or vendoring. Within this repo, `asset-allocation-contracts==3.4.0` is declared as the current exact shared-package dependency in `python/pyproject.toml`. CI verifies that the exact pin resolves from the configured package index and that built wheel and sdist metadata declare the same exact pin. Security verification also requires that the declared exact pin resolves before audit.
...
This package currently targets Python `>=3.14,<3.15` and runtime dependencies `azure-identity==1.25.2` and `httpx==0.28.1`. The repo also declares `asset-allocation-contracts==3.4.0` as the current exact shared-package dependency, and CI plus security verification require that the exact pin resolves. CI also validates that the built wheel and sdist metadata preserve the same exact pin. Test-only dependencies are declared separately.
...
| --- | --- | --- | --- |
| 2026-04-24 | Advance `asset-allocation-contracts` to the exact `3.4.0` release in source so runtime-common publishes against the latest stable shared package version. | 6, 8, 11, 13 | Active |
| 2026-04-24 | Add shared ACA job metadata validation/normalization and a producer-only strategy publication signal client; advance `asset-allocation-contracts` to the exact `3.3.0` release and runtime-common to `3.4.0` for downstream adoption. | 1, 2, 4, 6, 8, 11, 13 | Superseded |
