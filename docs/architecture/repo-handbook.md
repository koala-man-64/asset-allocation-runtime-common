...
- `asset-allocation-runtime-common` currently declares `asset-allocation-contracts==3.10.0` as its exact shared-package dependency, and CI verifies that the pin resolves from the configured package index and propagates into built distribution metadata.
