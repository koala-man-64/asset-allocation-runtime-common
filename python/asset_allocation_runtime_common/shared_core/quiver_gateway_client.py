...
class QuiverGatewayClient:
    ...
    def _build_headers(self) -> dict[str, str]:
        ...
        try:
            ...
            raise QuiverGatewayAuthError("Failed to acquire bearer token for the Asset Allocation API gateway.") from exc
    ...
    def _request_json(self, path: str, *, params: Optional[dict[str, Any]] = None) -> Any:
        ...
        try:
            response = self._http.get(url, headers=self._build_headers(), params={k: v for k, v in dict(params or {}).items() if v is not None})
        ...
        if response.status_code in {401, 403}:
            raise QuiverGatewayAuthError(detail or "Unauthorized.", status_code=response.status_code, detail=detail, payload=payload)
        if response.status_code == 404:
            raise QuiverGatewayNotFoundError(detail or "Not found.", status_code=response.status_code, detail=detail, payload=payload)
        if response.status_code == 429:
            raise QuiverGatewayRateLimitError(detail or "Rate limited.", status_code=response.status_code, detail=detail, payload=payload)
        raise QuiverGatewayUnavailableError(
